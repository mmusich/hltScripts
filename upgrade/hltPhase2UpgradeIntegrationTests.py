...
parser.add_argument("--globaltag", required=True, help="GlobalTag for the CMS conditions")
parser.add_argument("--geometry", required=True, help="Geometry setting for the CMS process")
parser.add_argument("--events", type=int, default=1, help="Number of events to process")
...
if args.globaltag is None or args.geometry is None:
    print_help()
    sys.exit(0)
...
base_cmsdriver_command = (
    f"cmsDriver.py step2 -s DIGI:pdigi_valid,L1TrackTrigger,L1,L1P2GT,DIGI2RAW,HLT:75e33_timing "
    f"--conditions {global_tag} --datatier GEN-SIM-DIGI-RAW -n {num_events} --eventcontent FEVTDEBUGHLT "
    f"--geometry {geometry} --era Phase2C17I13M9 --filein file:{output_dir}/step1.root --fileout {output_dir}/hlt.root --no_exec "
    f"--nThreads {num_threads}"
)

# The base configuration file and the dumped configuration file
base_config_file = os.path.join(output_dir, "step2_DIGI_L1TrackTrigger_L1_L1P2GT_DIGI2RAW_HLT.py")
dumped_config_file = os.path.join(output_dir, "step2_dump.py")
...
hlt_paths = re.findall(r"process\.(HLT_[A-Za-z0-9_]+|L1T_[A-Za-z0-9_]+)", config_content)
...
# Step 1: Run cmsDriver.py for TTbar GEN,SIM steps and save the output in output_dir
ttbar_config_file = os.path.join(output_dir, "TTbar_GEN_SIM_step.py")
ttbar_command = (
    f"cmsDriver.py TTbar_14TeV_TuneCP5_cfi -s GEN,SIM -n {num_events} "
    f"--conditions {global_tag} --beamspot DBrealisticHLLHC --datatier GEN-SIM "
    f"--eventcontent FEVTDEBUG --geometry {geometry} --era Phase2C17I13M9 "
...
# Step 2: Function to run cmsRun on a given HLT config file and save the output
def run_cmsrun(config_file):
    # Extract the HLT path name from the config file (e.g., "step2_HLT_IsoMu24_FromL1TkMuon.py")
    base_name = os.path.basename(config_file).replace("step2_", "").replace(".py", "")
...
config_files = [f for f in os.listdir(hlt_configs_dir) if f.endswith(".py") and f.startswith("step2_")]
...
# Step 4: Compare all HLT root files using hltDiff
def compare_hlt_results():
    # List all root files starting with "HLT_" in the output directory
    root_files = [f for f in os.listdir(output_dir) if f.endswith(".root") and f.startswith("HLT_")]
...
# Step 5: Once all cmsRun jobs are completed, perform the hltDiff comparisons
print("Performing HLT result comparisons...")
compare_hlt_results()

# Step 6: Capture the end time and print the end timestamp
end_time = time.time()
...
# Step 7: Calculate the total execution time and print it
