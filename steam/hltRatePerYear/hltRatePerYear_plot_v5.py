...
if __name__ == '__main__':
    ...
    for idx,year in enumerate(years):
        ...
        h1.GetXaxis().SetBinLabel(idx+1, str(year))
        h2.GetXaxis().SetBinLabel(idx+1, str(year))
    ...
    h1.GetYaxis().SetRangeUser(0.0001, 5)
    ...
    h2.GetYaxis().SetRangeUser(0.0001, 30)
    ...
    for idx,year in enumerate(years):
        ...
        fill_label_xmin = LEF + (1-LEF-RIG)/len(years) * (idx + 0.0000000018)
        txt1 = ROOT.TPaveText(fill_label_xmin, 0.09, fill_label_xmin+0.08, 0.15, 'NDC')
...
#    leg1.AddEntry(g_parking, 'Parking', 'f')
