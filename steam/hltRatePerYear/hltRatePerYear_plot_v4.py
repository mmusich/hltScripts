...
if __name__ == '__main__':
    ...
    use_2nd_axis = bool(int(sys.argv[2]))
    ...
    if use_2nd_axis:
        g_prompt.SetLineColor(38)
        g_prompt.SetFillColor(38)
#        g_prompt.SetFillStyle(3004)
    else:
    ...
    if use_2nd_axis:
        g_prompt.SetBarWidth(0.2)
        g_prompt.SetBarOffset(0.1)
        g_promptPlusParking.SetBarWidth(0.2)
        g_promptPlusParking.SetBarOffset(0.3)
        g_scouting.SetBarWidth(0.2)
        g_scouting.SetBarOffset(0.6)
    else:
        g_prompt.SetBarWidth(0.2)
        g_prompt.SetBarOffset(0.2)
        g_promptPlusParking.SetBarWidth(0.2)
        g_promptPlusParking.SetBarOffset(0.4)
        g_scouting.SetBarWidth(0.2)
        g_scouting.SetBarOffset(0.6)
    ...
    h0.GetYaxis().SetTitle('HLT Rate'+use_2nd_axis*' (Standard, Parking)'+' [kHz]')
    h0.GetYaxis().SetTitleSize(0.05 * (not use_2nd_axis))
    h0.GetYaxis().SetTitleOffset(1.0)
    h0.GetYaxis().SetRangeUser(0, prompt_ymax)
    h0.GetYaxis().SetNdivisions(nDivs)
    h0.GetYaxis().SetLabelSize( h0.GetYaxis().GetLabelSize() * 1.2 * (not use_2nd_axis) )
...
#    g_parking.Draw('same,bar0')
    ...
    leg.SetFillColor(0)
    leg.AddEntry(g_prompt, 'Standard', 'f')
#    leg.AddEntry(g_parking, 'Parking', 'f')
    leg.AddEntry(g_promptPlusParking, 'Standard + Parking', 'f')
    leg.AddEntry(g_scouting, 'Scouting', 'f')
    leg.Draw('same')
