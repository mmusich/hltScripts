...
if __name__ == '__main__':
    ...
    outputFileName = f'ratePlot_230109_all'
    ...
    lumi_ymax = 2
    lumi_factor = lumi_ymax / prompt_ymax
    ...
    for idx,year in enumerate(years):
        ...
        v_all += [[idx+0.5, v_prompt[-1][1] + v_parking[-1][1] + v_scouting[-1][1]]]
        v_lumi += [[idx+0.5, data[year]['Lumi'] / lumi_factor]]

    g_prompt = fill_tgraph('prompt', v_prompt)
    g_prompt.SetMarkerSize(1.2)
    g_prompt.SetMarkerStyle(24)
    g_prompt.SetMarkerColor(1)
    g_prompt.SetFillStyle(3005)
    ...
    g_scouting.SetLineStyle(2)
    g_scouting.SetLineWidth(2)
    g_scouting.SetFillStyle(3005)
    ...
    g_promptPlusParking.SetLineStyle(2)
    g_promptPlusParking.SetLineWidth(2)
    g_promptPlusParking.SetFillStyle(3005)
    g_promptPlusParking.SetFillColor(4)

    g_all = fill_tgraph('all', v_all)
    g_all.SetMarkerSize(1.2)
    g_all.SetMarkerStyle(21)
    g_all.SetMarkerColor(2)
    g_all.SetLineColor(2)
    g_all.SetLineStyle(2)
    g_all.SetLineWidth(2)
    g_all.SetFillStyle(3005)
    g_all.SetFillColor(2)

    g_lumi = fill_tgraph('lumi', v_lumi)
    g_lumi.SetMarkerSize(1.2)
    g_lumi.SetMarkerStyle(22)
    g_lumi.SetMarkerColor(ROOT.kViolet)
    g_lumi.SetLineColor(ROOT.kViolet)
...
#    ax1.Draw()
    ...
    ax2 = ROOT.TGaxis(7,0,7,prompt_ymax,0,lumi_ymax,nDivs,'+L')
    ax2.SetLineColor(ROOT.kViolet)
    ax2.SetLabelColor(ROOT.kViolet)
    ax2.SetTitleColor(ROOT.kViolet)
    ax2.SetTitle('Instantaneous Luminosity [10^{34} cm^{-2} s^{-1}]')
...
#        txt2.Draw('same')
    ...
    txt3 = ROOT.TPaveText(0.16, 0.73, 0.47, 0.85, 'NDC')
...
#    g_scouting.Draw('lp')
    ...
    leg = ROOT.TLegend(0.17, 0.51, 0.35, 0.71)
    leg.SetBorderSize(2)
    leg.SetTextFont(42)
...
#    leg.AddEntry(g_parking, 'Parking', 'lp')
    ...
    leg.AddEntry(g_all, 'Standard + Parking + Scouting', 'lp')
    leg.AddEntry(g_lumi, 'Instantaneous Luminosity', 'lp')
    leg.Draw('same')
