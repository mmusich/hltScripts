...
# git checkout -b HLTdevelopment130X_for133X && git commit -m 'HLT menu development for 13_0_X (branch: 13_3_X)' --allow-empty
# git checkout -b HLTdevelopment130X_for132X && git commit -m 'HLT menu development for 13_0_X (branch: 13_2_X)' --allow-empty
...
if [ "${branch}" = HLTdevelopment130X_for133X ]; then
  sshkey_enable
  cimsg='HLT menu development for 13_0_X (branch: 13_3_X)'
...
elif [ "${branch}" = HLTdevelopment130X_for132X ]; then
  [ $# -eq 1 ] || return 1
  sshkey_enable
  cimsg='HLT menu development for 13_0_X (branch: 13_2_X)'
  hash=$1
  cd "${CMSSW_BASE}"/src
  stormCherryPick
  cd "${CMSSW_BASE}"/src
