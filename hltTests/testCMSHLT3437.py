...
os.system('hltConfigFromDB --configName /dev/CMSSW_15_0_0/GRun/V10 > GRun.py')
...
for pathName in psDict:
  ...
  if cols_psSum == 0:
    ...
    os.system('grep "'+pathName+'_v*" '+os.environ['CMSSW_BASE']+'/src/HLTrigger/Configuration/tables/GRun.txt >> tmp.log')

os.system('awk \'{ print "  -", $3, ":", $1 }\' tmp.log | sed \'s|,||g\' -- | sed \'s|v\*|v|g\' -- | sort -u')
os.system('rm -f tmp.log')

ownersDict = json.load(open(os.environ['CMSSW_BASE']+'/src/HLTrigger/Configuration/scripts/utils/hltPathOwners.json'))
