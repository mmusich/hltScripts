...
#    print('-'*50)
    ...
    for fedId in range(4096+1):
        ...
        if 1200 <= fedId and fedId <= 1349:
            eventSizeDict['Pixel'] += s1

        elif 50 <= fedId and fedId <= 489:
            eventSizeDict['Strips'] += s1

        elif 520 <= fedId and fedId <= 575:
            eventSizeDict['PreShower'] += s1

        elif 600 <= fedId and fedId <= 670:
            eventSizeDict['ECAL'] += s1

        elif 1100 <= fedId and fedId <= 1199:
            eventSizeDict['HCAL'] += s1

        elif 750 <= fedId and fedId <= 869:
            eventSizeDict['Muon'] += s1

        elif 1369 <= fedId and fedId <= 1371:
            eventSizeDict['Muon'] += s1

        elif 1354 <= fedId and fedId <= 1405:
            eventSizeDict['L1T'] += s1

        else:
            eventSizeDict['Others'] += s1
