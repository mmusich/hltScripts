...
def process_file(each_root_file):
    ...
    process_of_interest = ["ZStoRECO", "RECO", "reRECO", "PAT", "NANO", "DQM", "HLT", "HLT2","HLTX"]
    ...
    print("ref_path:",ref_path,"fileName:",fileName)    
    path = "../base" #ref_path.replace(options.ref, options.base)
    print("path:",path)
    spec = "all"
    print(spec)
...
if __name__ == "__main__":
    ...
    all_output_root_files = glob.glob("%s/*.root" % (options.ref))

    print(all_output_root_files)
