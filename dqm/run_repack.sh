...
REFERENCE_REL=CMSSW_17_0_0_pre2
#REFERENCE_REL=CMSSW_20_1_X_2026-07-03-1100
TARGET_REL=CMSSW_20_1_X_2026-07-03-1100

INPUTFILE=$1
RUN=$2
STREAM=$3

if [ -z "$INPUTFILE" ] || [ -z "$RUN" ] || [ -z "$STREAM" ]; then
    echo "Usage: $0 <input_dat_file> <runNumber> <stream>"
    echo "Streams: DQMGPUvsCPU DQMOnlineScouting DQMTestDataScouting DQM HIDQM"
    exit 1
fi

case "$STREAM" in
    DQMGPUvsCPU|DQMOnlineScouting|DQMTestDataScouting|DQM|HIDQM)
        ;;
    *)
...
process.hltOutput${STREAM} = cms.OutputModule( "GlobalEvFOutputModule",
...
process.${STREAM}Output = cms.EndPath(process.hltOutput${STREAM})
EOF
